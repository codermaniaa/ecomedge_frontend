...
const Order = ({ params }) => {
  ...
  const { submitHandler, orderResponse} = useCheckoutSubmit();
    ...
    <Layout title="Invoice" description="order confirmation page">
      {loading && !data ? (
        ...
        <div className="max-w-screen-2xl mx-auto py-10 px-3 sm:px-6">
          <div className="bg-gray-300 rounded-md mb-5 px-4 py-3">
            <label>
                ...
                >
                  {/* {({ blob, url, loading, error }) =>
                    loading ? (
                      ...
                      <button className="mb-3 sm:mb-0 md:mb-0 lg:mb-0 flex items-center justify-center bg-gray-800  text-white transition-all font-serif text-sm font-semibold h-10 py-2 px-5 rounded-md">
                        {showingTranslateValue(
