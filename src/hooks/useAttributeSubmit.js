...
const useAttributeSubmit = (id) => {
  ...
  const onSubmit = async ({ title, name, option,variants }) => {
    try {
      ...
      console.log("Error response is here.... ",err.response);
  ...
  useEffect(() => {
    ...
    if (location.pathname === "/admin/attributes" && id) {
      (async () => {
        try {
          const res = await AttributeServices.getAttributeById(id);
          if (res) {
            console.log("Responses.......................... ",res.variants);
            setResData(res);
            ...
            if(res.variants && res.variants.length > 0){
              let allvariant = []
              res.variants.map( cur_variant=>{
                allvariant.push(cur_variant.name);
              })
