...
const MainCarousel = () => {
  const { storeCustomizationSetting, bannerData, getDetails } = useGetSetting();
          ...
          bannerData?.map((item, i) => (
            <dev>
              <SwiperSlide
                ...
                key={i + 1}
              >
                <div className="text-sm text-gray-600 w-full h-full hover:text-gray-dark">
                  <img
                    layout="responsive"
                    src={item?.image}
                    alt={item?.heading}
                ...
                <div className="absolute top-0 left-0 z-10 p-r-16 flex-col flex w-full h-full place-items-start justify-center">
                  <div className="pl-4 pr-12 sm:pl-10 sm:pr-16 w-10/12 lg:w-8/12 xl:w-7/12">
                    ...
                    <Link href={item?.image}>
                      <a className="hidden sm:inline-block lg:inline-block text-sm leading-6 font-serif font-medium mt-6 px-6 py-2 bg-gray-800 text-center rounded-md text-white hover:bg-gray-600">
                        {item?.ctaText.split("-").join(" ")}
                      </a>
                    </Link>
                  </div>
                </div>
